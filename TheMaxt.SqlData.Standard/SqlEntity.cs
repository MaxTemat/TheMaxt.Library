...
namespace TheMaxt.SqlData.Standard
{
	/// <summary>
	...
	public class SqlEntity<T> where T : new()
	{
		...
		public virtual void Set(T oldObject, T newObject)
		{
			try
			{
				...
				for (int i = 0; i < properties.Length; i++)
				{
					var oldValue = (properties[i].GetValue(oldObject)) != null ? properties[i].GetValue(oldObject) : null;
					var newValue = (properties[i].GetValue(newObject)) != null ? properties[i].GetValue(newObject) : null;
